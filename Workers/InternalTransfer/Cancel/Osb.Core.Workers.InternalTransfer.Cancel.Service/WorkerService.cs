...
namespace Osb.Core.Workers.InternalTransfer.Cancel
{
    public class WorkerService
    {
        ...
        public void Cancel()
        {
            ...
            IEnumerable<BusinessModel.InternalTransfer> internalTransferList = internalTransferService.FindInternalTransferListByStatus(InternalTransferStatus.PreCanceled);

            foreach (BusinessModel.InternalTransfer internalTransferToProcess in internalTransferList)
            {
                try
                {
                    internalTransferService.CancelInternalTransfer(internalTransferToProcess.InternalTransferId);
                }
                catch (Exception)
                {
